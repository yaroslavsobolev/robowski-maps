...
def model_of_yield_for_one_condition(index_in_df, pKa_COH2plus, K_tilde, k_forward_over_k_backward, k_backward):
    pKa_HBr = 6.6
...
def work_wrapper(packed_args):
    i, pKa_COH2plus, K_tilde, k_forward_over_k_backward, k_backward = packed_args
    return model_of_yield_for_one_condition(i, pKa_COH2plus, K_tilde, k_forward_over_k_backward, k_backward)


def model_of_yield_for_many_conditions(indices, pKa_COH2plus, K_tilde, k_forward_over_k_backward, k_backward):
    print(f'Evaluating yield model for many conditions... {pKa_COH2plus}, {K_tilde}, {k_forward_over_k_backward}, {k_backward}')
    # # nonparallel version
    ...
    # parallel version
    list_of_parameter_lists = [(i, pKa_COH2plus, K_tilde, k_forward_over_k_backward, k_backward) for i in indices]
...
if __name__ == '__main__':
    ...
    keq_fit = tuple([7.49545443, 6.24868772, 0.48257973, 0.88322705])
    keq_initials = np.load('keq_fits_with_errs.npy')
    for temp_index, temperature in enumerate(temperatures):
        ...
        # take initial guess from the previous fit iteration
        p0 = keq_initials[temp_index][0]
        # if temperature == 16:
        #     p0[1] = 2.26
        # elif temperature == 36:
        #     p0[1] = p0[1] * 1.64

        keq_fit, keq_err = fit_kinetic_model(indices_where_mask_is_true, do_plot=True, p0=p0)
