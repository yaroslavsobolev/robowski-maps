...
def concentrations_by_ph(water_concentration, Kw, substances, ph, return_Decimals=True):
    ...
    assert np.isclose(float(remaining_water) + float(oh_minus), float(h2o), rtol=1e-8), f'Water digit precision insufficient: {remaining_water} + {oh_minus} != {h2o}'
...
if __name__ == '__main__':
    ...
    for ph in np.linspace(-65, 90, 300):
        ...
        assert np.isclose(float(sd), float(sd2), rtol=1e-8), f'Concentration of deprotonated species differs: {sd} != {sd2}'
        assert np.isclose(float(sp), float(sp2), rtol=1e-8), f'Concentration of protonated species differs: {sp} != {sp2}'
    ...
    haha = input('haha')
    # bu = Acid(pKa=11, charge=1, conc=0.1)
    # ts = Acid(pKa=-2.8, charge=0, conc=0.1)
    # # dmfh = Acid(pKa=6, charge=1, conc=0.1)
    # # cl = Inert(charge=-1, conc=1e-8)
    # # system = System(hf, na)
    # system = System(bu, ts)
    # # hf = Acid(pKa=-1, charge=0, conc=1)
    # # na = Acid(pKa=17, charge=1, conc=0.01)
    # # system = System(hf)
    # system.pHsolve()
    # print(f'phcalc: ph = {system.pH}') # Should print 7.5992233
    ...
    for i, BuNH2 in tqdm(enumerate(BuNH2_concentrations), total=len(BuNH2_concentrations)):
        for j, TsOH in enumerate(TsOH_concentratuons):
            ...
            phs[i, j] = ph_here
            subs, remaining_water, oh_minus = concentrations_by_ph(water_concentration=TsOH, Kw=Kw_here, substances=substances,
                                                                   ph=ph_here)
    ...
    # import pickle
    # with open('misc-scripts/ugi_ph.pkl', 'wb') as f:
    #     pickle.dump(phs, f)
    # with open('misc-scripts/ugi_waters.pkl', 'wb') as f:
    #     pickle.dump(waters, f)
    # with open('misc-scripts/ugi_proportions.pkl', 'wb') as f:
    #     pickle.dump(proportions, f)
    #
    # # load from pickle
    # with open('misc-scripts/ugi_ph.pkl', 'rb') as f:
    #     phs = pickle.load(f)
    # with open('misc-scripts/ugi_waters.pkl', 'rb') as f:
    #     waters = pickle.load(f)
    # with open('misc-scripts/ugi_proportions.pkl', 'rb') as f:
    ...
    #
    plt.imshow(phs, extent=(BuNH2_concentrations.min(), BuNH2_concentrations.max(), TsOH_concentratuons.min(), TsOH_concentratuons.max()),
    ...
    plt.tight_layout()
    fig.savefig('misc-scripts/figures/ugi_ph.png', dpi=300)
    plt.show()

    fig, ax = plt.subplots(1, 1, figsize=(5, 4))
    # colorbar with title of 'pH'
    plt.imshow(proportions, extent=(BuNH2_concentrations.min(), BuNH2_concentrations.max(), TsOH_concentratuons.min(), TsOH_concentratuons.max()),
                  interpolation=interpolation_method, origin='lower')
    plt.colorbar(label='Fraction of butylammonium to\nthe sum of butylammonium and butylamine')
    plt.xlabel('(p-TSA$\cdot H_2 O$) concentration, mol/L')
    plt.ylabel('Total amine+ammonium concentration, mol/L')
    plt.tight_layout()
    fig.savefig('misc-scripts/figures/ugi_ion_fraction.png', dpi=300)
    plt.show()

    fig, ax = plt.subplots(1, 1, figsize=(5, 4))
    plt.imshow(waters, extent=(BuNH2_concentrations.min(), BuNH2_concentrations.max(), TsOH_concentratuons.min(), TsOH_concentratuons.max()),
                  interpolation=interpolation_method, origin='lower')
    # colorbar with title of 'pH'
    ...
    plt.tight_layout()
    fig.savefig('misc-scripts/figures/ugi_neutral_water.png', dpi=300)
    plt.show()