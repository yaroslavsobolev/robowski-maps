...
logger_path = data_folder[:-5] + 'pipetter_files/main_roboski1.log'
...
if __name__ == '__main__':
    ...
    #manually specify the volumes for each steps
    ...
    ##摆烂版本，第二步稀释的体积太难计算了，以下是第二步系数为200时的三个体积
    ## this is the actual volume that will be used.
    # volumes = [1000,10,990,0,150,300] # Wai Shing Tetrahydropyrimidine
    # volumes = [600,50,1450,0,50,1450]
    # volumes = [1000,30,1970,0,0,0]
    volumes = [1000,40,960,0,50,950] #Biginelli
    ...
    for index, event in enumerate(events_B_to_C[1]): # construct the second step for B to C
        ...
        event.source_container.liquid_surface_height = 2110
        event.transfer_volume = 50
        event.liquidClassTableIndex = 12
        event.tip_type = '50ul'
