...
def spectrum_to_concentration_local(sp, target_spectrum_input, calibration_folder, calibrant_shortnames,
    ...
    label_calibrant_shortnames = ['Product', 'Substrate']

    ax.plot(wavelengths, func(wavelength_indices, *popt), '-', color='black', label='Fitted model', alpha=1)
    ...
    ax.set_ylim(0, 0.25)
    ax.set_xlim(min(wavelengths[mask]), 340)
...
calibration_folder = data_folder + experiment_name + 'microspectrometer_data/calibration/'
process_plate(sp, dilution_factor=500,
              plate_folder=f'{data_folder}{experiment_name}2024-02-15_15-03-54_UV-Vis_plate_89.csv',
              well_ids=[4,5],
              cut_from=15,
              cut_to=250,
