...
def plot_dft_vs_pink(gaussian_file, molar_concentration, legend=True, ymax=0.35, showmolar=False, ev_range=(1.3, 5)):
    ...
    np.save(f'misc-scripts/figures_for_articles/dft-uv-vis/{gaussian_file.split("/")[-1].replace(".out", "_nms.npy")}', nms)
    np.save(f'misc-scripts/figures_for_articles/dft-uv-vis/{gaussian_file.split("/")[-1].replace(".out", "_mean.npy")}',
            spectrum * molar_concentration)
    np.save(f'misc-scripts/figures_for_articles/dft-uv-vis/{gaussian_file.split("/")[-1].replace(".out", "_perc1.npy")}',
            percentiles[0] * molar_concentration)
    np.save(f'misc-scripts/figures_for_articles/dft-uv-vis/{gaussian_file.split("/")[-1].replace(".out", "_perc2.npy")}',
...
def plot_dft_vs_pink_weighted(freqfiles, uvfiles, molar_concentration, legend=True, ymax=0.35, showmolar=False, ev_range=(1.3, 5)):
    ...
    st.simpleaxis(ax)
    # plt.tight_layout()
    fig.savefig(f'misc-scripts/figures_for_articles/dft-uv-vis/{uvfiles[0].split("/")[-1].replace(".out", ".png")}')



if __name__ == '__main__':
    base_folder = 'misc-scripts/dft_molar-absorptivity-spectra/dft-calculation-results/candidates/dimer_OHplus/'
    plot_dft_vs_pink(gaussian_file = base_folder + 'dimer_OHplus_conf2_1054kjm_uv.out',
                     molar_concentration=1/(1e9*0.6/1.20*1.63),
                     legend=False)
    plt.show()

    base_folder = 'misc-scripts/dft_molar-absorptivity-spectra/dft-calculation-results/candidates/other_candidates_uv/'
