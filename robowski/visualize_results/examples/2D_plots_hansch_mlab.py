...
for dataset_id, data_point in enumerate(data):
    ...
    if what_to_plot == 'data':
        for idx in range(len(xs0.flatten())):
            mlab.points3d(xs0.flatten()[idx], ys0.flatten()[idx], zs0.flatten()[idx], color=error_based_colors[idx], scale_factor=0.08)

        plot = mlab.points3d(xs0.flatten(), ys0.flatten(), zs0.flatten(), color=vivid_color_here, scale_factor=0.0008)
...
if do_smooth:
    ...
    smooth_suffix = ''
scene.scene.save(repo_data_path + f'misc_scripts/figures/hantzsch-points3d-{what_to_plot}{smooth_suffix}.png')
