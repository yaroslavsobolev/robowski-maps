...
def plot_3d_dataset_as_cube(x_raw, y_raw, z_raw, k_raw, s_raw, substance_titles = ('Alcohol', 'HBr', 'Temperature'),
    ...
    if interpolator_choice == 'rbf':
        ...
        interp_here = LinearNDInterpolator((xs0[:154], ys0[:154], zs0[:154]), ks0[:154])
    ...
    if forced_kmax is not None:
        ...
        ks_scaled = (ks - np.min(ks)) / (np.max(ks) - np.min(ks))
        pts = mlab.quiver3d(xs, ys, zs, s_raw, s_raw, s_raw, scalars=ks_scaled, mode="sphere", scale_factor=0.3,
                            colormap=colormap,
                            opacity=0.03, vmax=np.max((ks0 - np.min(ks)) / (np.max(ks) - np.min(ks))))
...
df_results.loc[df_results['yield'] == np.inf, 'yield'] = 1.0
...
for substrate in substances:
    # remove all rows where substrate is below 119
    df_results = df_results[df_results[substrate] >= 119]
...
# sraw = np.ones_like(yields) * 0.1  # just a dummy value for the sake of the example
sraw = df_results['ptsa'].to_numpy()

plot_3d_dataset_as_cube(xs, ys, zs, yields, sraw,
                            substance_titles=['Amine,\nmM', 'Isocyanide,\nmM', 'Aldehyde,\nmM'],
                            ...
                            contours=[0.1, 0.70, 0.93, 0.98], contour_opacity=0.005)
