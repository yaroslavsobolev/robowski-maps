...
if 'ROBOCHEM_DATA_PATH' in os.environ:
    ...
    import importlib
    abe = importlib.import_module("visualize-results.examples.kinetics_models.acid_base_equilibrium")
...
def model_of_yield_for_many_conditions(indices, pKa_COH2plus_deltaS, pKa_COH2plus_deltaH, K_tilde_deltaS, K_tilde_deltaH, k_forward_over_k_backward_deltaS, k_forward_over_k_backward_deltaH, k_backward_kappa, k_backward_deltaS, k_backward_deltaH, HBr_B):
    ...
    with Pool(njobs) as pool:
        model = list(tqdm(pool.imap(work_wrapper, list_of_parameter_lists, chunksize=1), total=len(indices)))
    return model


def model_of_yield_for_one_condition_eq(index_in_df, pKa_COH2plus_deltaS, pKa_COH2plus_deltaH, K_tilde_deltaS, K_tilde_deltaH, k_forward_over_k_backward_deltaS, k_forward_over_k_backward_deltaH, k_backward_kappa, k_backward_deltaS, k_backward_deltaH, HBr_B):
    # get row of df_results with index_in_df
    row = df_results.loc[index_in_df]
    # pKa_HBr = 6.6
    ...
    # use solve_ivp to solve the system of ODEs
    sol = solve_ivp(right_hand_side, (0, reaction_time_in_hours), y0, t_eval=[reaction_time_in_hours],
                    method='RK45', first_step=0.000001, max_step=1000, atol=1e-8, rtol=1e-7)
    # sol = solve_ivp(right_hand_side, (0, reaction_time_in_hours), y0, t_eval=[reaction_time_in_hours],
    #                 method='RK45', first_step=0.01, max_step=2, atol=1e-4, rtol=1e-3)
    c_product = sol.y[1][-1]
...
def work_wrapper_eq(packed_args):
    i, pKa_COH2plus_deltaS, pKa_COH2plus_deltaH, K_tilde_deltaS, K_tilde_deltaH, k_forward_over_k_backward_deltaS, k_forward_over_k_backward_deltaH, k_backward_kappa, k_backward_deltaS, k_backward_deltaH, HBr_B = packed_args
    return model_of_yield_for_one_condition_eq(i, pKa_COH2plus_deltaS, pKa_COH2plus_deltaH, K_tilde_deltaS, K_tilde_deltaH, k_forward_over_k_backward_deltaS, k_forward_over_k_backward_deltaH, k_backward_kappa, k_backward_deltaS, k_backward_deltaH, HBr_B)


def model_of_yield_for_many_conditions_eq(indices, pKa_COH2plus_deltaS, pKa_COH2plus_deltaH, K_tilde_deltaS, K_tilde_deltaH, k_forward_over_k_backward_deltaS, k_forward_over_k_backward_deltaH, k_backward_kappa, k_backward_deltaS, k_backward_deltaH, HBr_B):
    print(f'Evaluating yield model for many conditions, params are {pKa_COH2plus_deltaS, pKa_COH2plus_deltaH, K_tilde_deltaS, K_tilde_deltaH, k_forward_over_k_backward_deltaS, k_forward_over_k_backward_deltaH, k_backward_kappa, k_backward_deltaS, k_backward_deltaH, HBr_B}')
    # # nonparallel version
    # return [model_of_yield_for_one_condition(index_in_df=i, pKa_HBr=pKa_HBr, pKa_COH2plus=pKa_COH2plus,
    #                                          K_tilde=K_tilde, k_forward_over_k_backward=k_forward_over_k_backward,
    ...
    list_of_parameter_lists = [(i, pKa_COH2plus_deltaS, pKa_COH2plus_deltaH, K_tilde_deltaS, K_tilde_deltaH, k_forward_over_k_backward_deltaS, k_forward_over_k_backward_deltaH, k_backward_kappa, k_backward_deltaS, k_backward_deltaH, HBr_B) for i in indices]
    with Pool(njobs) as pool:
        model = list(tqdm(pool.imap(work_wrapper_eq, list_of_parameter_lists, chunksize=1), total=len(indices)))
    return model
...
def fit_kinetic_model(indices_here, do_plot=False,
    ...
    def produce_fit(x, y):
        ...
        popt, pcov = curve_fit(model_of_yield_for_many_conditions, x, y, p0=p0,
                               ...
                               x_scale=[x for x in [45, 10, 150, 50, 1, 1, 1e-6, 1, 1, 0.1]],
                               diff_step=[x/10 for x in ([0.01] + [0.01] + [0.005] + [0.005] + [0.01]*5 + [0.1])],
...
def plot_kinetic_model(indices_here, popt):
    ...
    measured_yields = df_results.loc[indices_here, 'yield']
    # plt.scatter(xs_to_plot, measured_yields, color='yellow', marker='o')
    f = best_f = lambda x: list(np.array(model_of_yield_for_many_conditions(x, *popt)) / np.array(model_of_yield_for_many_conditions_eq(x, *popt)))
    keq_fit = popt
    do_plot = True
    if do_plot:
        # plt.scatter(xs_to_plot, measured_yields, color=colors_to_plot, alpha=0.5, s=10)
        ...
        simpleaxis(plt.gca())
        plt.gca().yaxis.set_major_formatter(mtick.PercentFormatter(1.0))
        plt.ylabel('Fraction of yield to equilibrium yield')
        plt.xlabel('Starting concentration of HBr, M')
        plt.tight_layout()
        plt.legend(title="Starting alcohol\nconcentration")
...
if __name__ == '__main__':
    ...
    # Load bootstrapping dataset and evaluate the median parameters
    samples = np.load(
        f'{data_folder}simple-reactions/2023-09-14-run01/results/kinetics/bootstrapped_keq_fits_size59_2024-12-17c_temps16-21-26.npy')
    samples_2 = np.load(
        f'{data_folder}simple-reactions/2023-09-14-run01/results/kinetics/bootstrapped_keq_fits_size305_2024-12-24a_temps16-21-26.npy')
    samples = np.concatenate((samples, samples_2), axis=0)
    print(f'Shape of samples: {samples.shape}')
    keq_fit = np.median(samples, axis=0)
    print(f'Truths: {keq_fit}')
    np.save(f'best_fit_params.npy', keq_fit)

    keq_fit = np.load('best_fit_params.npy')
    ...
    for temp_index, temperature in enumerate(temperatures):
        ...
        plt.ylim(0, 1.1)
        plt.gcf().savefig(
            f'eq_fraction_at_temperature_{temperature}C.png',
            dpi=300)
        # close all figs
        plt.close('all')
