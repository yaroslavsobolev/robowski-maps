...
This particular script implements the model in the full version with the 'hydrative' mechanism.
'''
...
def model_of_yield_for_one_condition(*popt):
    ...
    def right_hand_side_of_ode(t, y):
        ...
        c_a, c_b, c_c, c_p1, c_p2, c_p3, c_p4, c_me2nh2, c_q1, c_q2, c_q4, c_m1, c_m4, c_h2o, c_a_h2o, c_18x, c_18z, c_product = y_corrected
        dc_a = dc_b = dc_c = dc_p1 = dc_p2 = dc_p3 = dc_p4 = dc_me2nh2 = dc_q1 = dc_q2 = dc_q4 = dc_m1 = dc_m4 = dc_h2o = dc_a_h2o = dc_18x = dc_18z = dc_product = 0
        ...
        substances = ({'pKa': pKa_BuNH3, 'conc': c_b, 'charge': 1},  # ammonium
                      ...
                      {'pKa': pKa_18x, 'conc': c_18x, 'charge': 1},
                      {'pKa': pKa_18z, 'conc': c_18z, 'charge': 1}
                      )
        ...
        # TODO: Should be Kw_here = Decimal(10) ** Decimal(-27.44) / (abe.PURE_WATER_MOLARITY) ** 2
        Kw_here = Decimal(10) ** Decimal(-27.44) / (abe.PURE_WATER_MOLARITY) ** 2
        # OLD VERSION
        # Kw_here = Decimal(10) ** Decimal(-31.4) / (abe.PURE_WATER_MOLARITY) / Decimal(13)
        try:
        ...
        # P_4 + H2O -> 18x
        forward_reaction_rate = k_10 * c_p4 * remaining_water
        net_reaction_rate = forward_reaction_rate
        dc_p4 += -net_reaction_rate
        dc_h2o += -net_reaction_rate
        dc_18x += net_reaction_rate

        # 18x(deprotonated) + C -> 18z
        forward_reaction_rate = k_11 * eq_18x['conc_deprot'] * c_c
        net_reaction_rate = forward_reaction_rate
        dc_18x += -net_reaction_rate
        dc_c += -net_reaction_rate
        dc_18z += net_reaction_rate

        # 18z (prot) -> Product
        forward_reaction_rate = k_12 * eq_18z['conc_prot']
        net_reaction_rate = forward_reaction_rate
        dc_18z += -net_reaction_rate
        dc_product += net_reaction_rate


        res = tuple([dc_a, dc_b, dc_c, dc_p1, dc_p2, dc_p3, dc_p4, dc_me2nh2, dc_q1, dc_q2, dc_q4, dc_m1, dc_m4, dc_h2o, dc_a_h2o, dc_18x, dc_18z, dc_product])
        res = np.array(res)
    ...
    y0 = [starting_zero_concentration_filler] * 18
...
def print_parameters_with_names(theta):
    parameter_names = ['k1', 'k_1', 'k2', 'k_2', 'k3', 'k_3', 'k4', 'k_4', 'k5', 'k6', 'k_6', 'k7', 'k8', 'k9', 'km1', 'k_m1', 'km3', 'pKa_ptsa', 'pKa_BuNH3', 'pKa_DMFH', 'pKa_p1', 'pKa_p2', 'pKa_p3', 'pKa_c', 'pKa_ch', 'pKa_q1', 'pKa_q2h', 'pKa_m1h', 'pKa_m4h', 'pKa_me2nh2', 'pKa_q4', 'pKa_18x', 'pKa_18z', 'k_10', 'k_11', 'k_12']
    for i, name in enumerate(parameter_names):
        print(f'{name}: {theta[i]}')


lower_bounds = [0] * 17     + [-8] * 14 + [-8] * 2 + [0] * 3
upper_bounds = [10000] * 17 +  [20] * 14 + [20] * 2 + [15000] * 3
...
lower_bounds[31] = -3 # pKa_18x
upper_bounds[31] = 4 # pKa_18x
lower_bounds[32] = 9 # pKa_18z
upper_bounds[32] = 17 # pKa_18z
lower_bounds[33] = 0 # k_10
upper_bounds[33] = 1000000 # k_10
lower_bounds[34] = 0 # k_11
upper_bounds[34] = 100000 # k_11
lower_bounds[35] = 0 # k_12
upper_bounds[35] = 100000 # k_12
...
def log_likelihood_parallel(theta):
    ...
    np.save('ugi_v4_theta_intermediate_during_opt.npy', theta)
    assert len(theta) == 36
...
def residuals_parallel(theta):
    ...
    np.save('ugi_v4_theta_intermediate_during_opt_v4.npy', theta)
    assert len(theta) == 36
    list_of_parameter_lists = [[i] + list(theta) for i in indices_here]
    with Pool(77) as pool:
        model = list(tqdm(pool.imap(work_wrapper, list_of_parameter_lists), total=len(indices_here)))
...
def plot_yields(do_load=False, filename='theta_max.npy', theta=None, do_show=False, sizefactor=5, show_legends=True, same_ylims=False):
    ...
    for i, indices in enumerate(list_of_index_sets):
        ...
        if not do_load:
            ...
            for k, ys in enumerate(ys_all):
                if indices_here[k] in indices:
                    ys2.append(df_results.loc[indices_here[k]]['yield'])
                    xs.append(df_results.loc[indices_here[k]]['ptsa'])
            np.save(f'ys1_{i}.npy', ys1)
            np.save(f'ys2_{i}.npy', ys2)
        else:
            ys1 = np.load(f'ys1_{i}.npy')
