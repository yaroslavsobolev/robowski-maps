...
absolute_path_to_temp_folder = 'C:\\robochem\\uv_vis_absorption_spectroscopy\\craic_automation\\temp_folder'
darkpos = (-5510, 0)

desktop_icons = {'lambdafire': ( 645,   37),
                 'stage_control': ( 567,   32)}

goto_window = {'ref_default': (1176, 104),
                     'ref_now': (1176, 104),
                     'x': (1317,   79),
                     'y': (1318,  120),
                     'goto': (1176, 104)}

main_window = {'ref_default': (1760, 583),
               'ref_now': (1760, 583),
               'collect_dark_and_ref': (1045, 1107),
               'collect_spectrum': (1277, 1111),
               'beamsplitter_one': (1425,  989),
               'clear_all': ( 752,  580),
               'my_folder': ( 561, 1103),
               'spectrometer_params': ( 921,  577),
               'camera_params': (1000,  582),
               # 'beamsplitter_one': (1425, 989),
               # 'beamsplitter_one': (1425, 989),
               # 'beamsplitter_one': (1425, 989),
               # 'beamsplitter_one': (1425, 989),
...
popup_window_buttons = {'ref_default': (0, 0),
                        ...
                        'enter_file_name': (1044,  629),
                        'scan_and_take_image': ( 953,  675),
                        # 'x': (0, 0),
                        # 'x': (0, 0),
                        # 'x': (0, 0),
                        # 'x': (0, 0),
...
def collect_spectrum(id, suffix=''):
...
def copy_temp_folder_to_archive(plate_id, exp_name):
    ...
    shutil.copytree(absolute_path_to_temp_folder, data_folder + 'craic_microspectrometer_measurements/absorbance/' + target_folder_name)
    df = pd.read_csv(data_folder + 'craic_microspectrometer_measurements/absorbance/database_about_these_folders.csv')
    df = df.append({'timestamp':unixtime, 'datetime':datetime_string_now, 'plate_id':plate_id, 'exp_name':exp_name, 'folder':target_folder_name},
                   ignore_index=True)
    df.to_csv(data_folder + 'craic_microspectrometer_measurements/absorbance/database_about_these_folders.csv',
              index=False)
...
def measure_one_plate(id, experiment_name='test',
    ...
    ys = np.linspace(0, y_range, nwells_y)
    well_id = 1
    endtimes = []
    starttimes = []
    starttimes_rep2 = []
    ...
    for x in xs:
        for y in ys:
            move_to((x, y))
            t0 = time.time()
            starttimes.append(collect_spectrum(id=well_id, suffix='') - t0)
            endtimes.append(time.time() - t0)

            # second repetition
            starttimes_rep2.append(collect_spectrum(id=well_id, suffix='_rep2') - t0)
            endtimes_rep2.append(time.time() - t0)
            well_id += 1
    ...
    np.savetxt(absolute_path_to_temp_folder + '\\starttimes_rep2.txt', np.array(starttimes_rep2))
    np.savetxt(absolute_path_to_temp_folder + '\\endtimes_rep2.txt', np.array(endtimes_rep2))
...
if __name__ == '__main__':
    experiment_name = \
'simple_reactions_2023-08-18-testing'
    click_button(main_window, 'clear_all')
    click_button(main_window, 'clear_all')
    ...
    while True:
        ...
        # UNCOMMENT IF YOU WANT TO SCAN BARCODES WITH WEBCAM
        ...
        decoded_info = input('Enter plate barcode: ')
        scanned_barcodes.pop(0)
        scanned_barcodes.append(decoded_info)
